...
#define kIsIpad (UIUserInterfaceIdiomPad == [[UIDevice currentDevice]userInterfaceIdiom ])
...
#define kTabHeight (kIsIpad? 80 :44.0)
#define kTabOffset 56.0
#define kTabWidth 128.0   //CGRectGetWidth(self.view.frame)/3
#define kTabWidth (CGRectGetWidth(self.view.frame)/self.tabCount)
...
#define kIndicatorColor [UIColor colorWithRed:30.0/255.0 green:189.0/255.0 blue:249.0/255.0 alpha:0.75]
#define kContentViewBackgroundColor [UIColor clearColor]
#define kTabsViewBackgroundColor [UIColor colorWithRed:11.0/255.0 green:43.0/255.0 blue:65.0/255.0 alpha:0.4]
...
@property CGFloat heightContentView;
...
- (void)layoutSubviews
{
    ...
    if (IOS_VERSION_7)
    {
        ...
        if (self.navigationController && !self.navigationController.navigationBarHidden)
        {
            ...
            NSLog(@"IOS_BERSION_7 topLaypotGuide : %f", topLayoutGuide);
        }
    }
    ...
    if (kIsIpad)
    {
        frame.origin.y = topLayoutGuide + 130;
    }
    else
    {
        frame.origin.y = [self.tabLocation boolValue] ? topLayoutGuide + CGRectGetHeight(self.tabsView.frame) : topLayoutGuide;
        NSLog(@"content view y :%f", frame.origin.y);
    }
...
//    self.contentView.backgroundColor = [UIColor redColor];
    ...
    for (TabView *tabMy in self.tabs)
    {
        if(tabMy.selected)
        {
            NSArray *subviews1 = [tabMy  subviews];
            if ([subviews1 count] > 0)
            {
                for (UILabel *subview1 in subviews1)
                {
                    [subview1 setTextColor:[UIColor colorWithRed:173.0/255.0 green:25.0/255.0 blue:31.0/255.0 alpha:1]];
                }
            }
        }
    }
...
- (NSNumber *)tabLocation {
    ...
    if (!_tabLocation)
    {
        ...
        NSLog(@"if in tabLocation : %f", value);
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
...
- (void)defaultSetup
{
    ...
    UIImage *imgBG = [UIImage imageNamed:@"devotionDDA"];
      UIImageView *backgroundView = [[UIImageView alloc] initWithImage:imgBG];
    backgroundView.frame = self.view.bounds;
    backgroundView.contentMode = UIViewContentModeScaleToFill;
    [self.view insertSubview:backgroundView atIndex:0];
    
//    
//    UIView  *bannerView1 = [[UIView alloc] initWithFrame:CGRectMake(0.0, self.view.frame.size.height-150, self.view.frame.size.width, 50)];
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        {
            tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, (self.view.frame.size.width / 3), 60)];
        }
...
//            view.frame = pageView.bounds;
            ...
            self.heightContentView = view.frame.size.height;
            
            viewController = [UIViewController new];
            [viewController.view addSubview:view]; // = view;
            // viewController.view.backgroundColor = [UIColor colorWithRed:11.0/255.0 green:43.0/255.0 blue:65.0/255.0 alpha:0.75];
        }
        else
        {
            viewController = [[UIViewController alloc] init];
